// Ethiopia Gospel Mission Database (Optimized for GitHub Server Hosting v17000)
...
if (typeof window !== 'undefined') {
  ...
  window.DATA_VERSION = "20260904_V17000_PPT_HTML_BASE64_PASTE_AND_OPTIMAL_CROPPER";
  ...
  window.db = {
    getMembers() {
      try {
        const stored = localStorage.getItem("ethiopia_members_v17000");
        if (stored) {
    ...
    saveMembers(mems) {
      try {
        if (Array.isArray(mems) && mems.length > 0) {
          localStorage.setItem("ethiopia_members_v17000", JSON.stringify(mems));
        }
      } catch(e) {}
    },
    getHistory() {
      try {
        const stored = localStorage.getItem("ethiopia_history_v17000");
        if (stored) {
          const parsed = JSON.parse(stored);
          if (Array.isArray(parsed) && parsed.length > 0) return parsed;
        }
    ...
    saveHistory(hists) {
      try {
        if (Array.isArray(hists) && hists.length > 0) {
          localStorage.setItem("ethiopia_history_v17000", JSON.stringify(hists));
        }
      } catch(e) {}
    },
    getFellowship() {
      try {
        const stored = localStorage.getItem("ethiopia_assemblies_v17000");
        if (stored) {
          const parsed = JSON.parse(stored);
    ...
    saveFellowship(items) {
      try {
        localStorage.setItem("ethiopia_assemblies_v17000", JSON.stringify(items));
      } catch(e) {}
    },
    getEvents() {
      try {
        const stored = localStorage.getItem("ethiopia_events_v17000");
        if (stored) {
          const parsed = JSON.parse(stored);
          if (Array.isArray(parsed)) return parsed;
        }
    ...
    saveEvents(evts) {
      try {
        localStorage.setItem("ethiopia_events_v17000", JSON.stringify(evts));
      } catch(e) {}
    },
    addEvent(e) {
